...
async function findOrgId(event: any) {
  ...
  return obj?.metadata?.organization_id || obj?.client_reference_id || obj?.subscription_details?.metadata?.organization_id || null;
}

export async function GET() {
  return NextResponse.json({ status: 'prepared', message: 'Stripe webhook endpoint is ready. Configure it in Stripe Dashboard and set STRIPE_WEBHOOK_SECRET.' });
}
...
export async function POST(request: Request) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    update.billing_email = obj.customer_details?.email || obj.customer_email || null;
  ...
  if (event.type === 'customer.subscription.updated') {
    ...
    update.stripe_customer_id = obj.customer;
    if (obj.current_period_end) update.current_period_end = new Date(obj.current_period_end * 1000).toISOString();
    if (obj.metadata?.plan) update.subscription_plan = obj.metadata.plan;
  ...
  if (Object.keys(update).length > 0) {
    ...
    else if (update.stripe_customer_id) query = query.eq('stripe_customer_id', update.stripe_customer_id);
    else if (update.stripe_subscription_id) query = query.eq('stripe_subscription_id', update.stripe_subscription_id);
    await query;
  }
